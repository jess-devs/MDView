...
| CA-02.4 — `<a href>` distinguible y clicable como un enlace Markdown | pasa | Captura: «enlace desde HTML» subrayado, igual que un enlace Markdown. La apertura del navegador no la pudo capturar esta sesión (ver nota de proceso), así que la verificó el usuario directamente: confirmó que un solo clic abre la redirección a `https://example.com/desde-html`, la URL exacta del `href` de prueba. |
...
**Nota de proceso — por qué CA-02.4 lo verificó el usuario, no esta
sesión.** Esta sesión intentó el clic dos veces: la primera,
`textinputhost.exe` (proceso de Windows para teclado táctil/IME) bloqueó el
clic sobre la ventana de MDView; la segunda, el clic sí llegó y aparecieron
procesos nuevos de Dia justo después, pero confirmar qué pestaña abrió
exigía traer la ventana del navegador al frente, y el permiso sobre la
barra de tareas («File Explorer») que eso requiere fue denegado. Entre
medias, `historias.md` registró HU-02 como «en curso», no verificada, y
siguió con HU-03 a continuación como una desviación consciente del método
de `00-contexto.md` bajo la delegación de esta sesión —anotada, no
escondida—, en vez de detener el resto de la feature por un único clic
pendiente. El usuario cerró el hueco directamente: un clic abre
`https://example.com/desde-html`, la URL exacta del `href` de prueba. El
mecanismo en sí (`Span.url` → `InteractiveText::on_click` →
`app::activate_link`) es código sin cambios respecto al ya verificado en
HU-02 de `enlaces-e-imagenes`; lo único nuevo en esta historia es extraer
el `href` de un atributo HTML, cubierto además por
`inline_html_a_carries_the_href_as_url`.

**Estado de la historia:** verificada. Los siete criterios pasan por
observación — seis por esta sesión, CA-02.4 por el usuario.
